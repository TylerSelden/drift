...
export function PBody(mass, shape, { linearDamping = 0.1, angularDamping = 0.2, type = CANNON.Body.DYNAMIC } = {}) {
  return new CANNON.Body({ mass, shape, linearDamping, angularDamping, type, position: new CANNON.Vec3(0, 0, 0) });
}

export function PBox({ size = [0.1, 0.1, 0.1], mass = 1 } = {}) {
  size = size.map(val => val / 2);

  const shape = new CANNON.Box(new CANNON.Vec3(...size));
  return PBody(mass, shape);
}

export function PSphere({ radius = 0.1, mass = 1 } = {}) {
  const shape = new CANNON.Sphere(radius);
  return PBody(mass, shape);
}

export function PCylinder({ rTop = 0.1, rBot = 0.1, height = 0.5, segments = 20, mass = 1 } = {}) {
...
export function PPill({ radius = 0.1, height = 0.1, segments = 32, mass = 1 } = {}) {
  ...
  const body = PBody(mass, cylinder);
  body.addShape(sphere, new CANNON.Vec3(0, height / 2 - radius, 0));
  body.addShape(sphere, new CANNON.Vec3(0, -height / 2 + radius, 0));
